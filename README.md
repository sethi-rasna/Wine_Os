...
Through the use of Machine Learning models of sklearn and TensorFlow, we will aim to predict Vinho Verde’s healthiness and quality based on chemical compositions. Healthiness will be determined by sulfate levels while quality will found through acidity and alcohol content. The plan for our machine learning model using TensorFlow implements input variables as red and white wine data. The output will determine which wines are highest in quality using the neural network nodes as metrics of fixed acidity, alcohol content, sulfites, and residual sugars. First we imported our dependencies then we processed the data by droping unnecessary variables and determined the number of unique values in each column. Next we binned the winetype by numeric value and generated our categorical variable lists. Then we generated our categorical variable lists. Importantly we defined "healthy" and "qaulity" wine by creating a column with metric scored based on suplhate content that was binary (1,2) and easy to decipher. We created a standardScaler fit and scaled the data preparing to compile, train, and evaluate the model. The model consisted of a deep learning net a neural net was sequentialy : first hidden layer, second hidden layer output layerand checked the structure of the model for a summary: 
